...
class AgentWrapper:
    ...
    def _ensure_remote_action_client(self, server_timeout_s: float = DEFAULT_AGENT_TIMEOUT_S):
        ...
        client = ActionClient(
            ...
            action_name=self._config.get("action_name", f"{self._name}_actions"),
            goal_host=host,
            goal_port=goal_port,
            feedback_host=host,
        ...
        wait_timeout_ms = int(max(0.5, server_timeout_s) * 1000)
        if not client.wait_for_server(timeout=wait_timeout_ms):
            try:
                client.close()
            except Exception:
                pass
    ...
    def _run_remote_agent_cmd(
        ...
        try:
            client = self._ensure_remote_action_client(timeout)
...
def _install_echo_python_path(root: Path) -> None:
    ...
    candidates.extend([
        root.parent / "echo_message_system" / "python",
        root / "third_party" / "echo_message_system" / "python",
    ])
    for echo_python_path in candidates:
        if echo_python_path.exists():
...
def _probe_agent(
    ...
    try:
        result = wrapper.cmd("check", _probe_params_for_agent(wrapper.name, script_path), timeout=timeout)
